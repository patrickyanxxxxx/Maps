...
- 默认使用 `CN_POI`：中国地点、POI 和反向地理编码使用 CN 服务。所有客户端默认在中国大陆使用高德导航，国外仍使用 TomTom。
...
除 Egern 外的模块发布和缓存版本为 `6.4.0`；Egern 继续使用此前验证通过的固定响应脚本。脚本内部仍显示原项目基础 bundle 版本 `4.6.1`，这是正常现象。
...
| `UrlInfoSet.Directions` | `AutoNavi` | 中国大陆使用高德导航和 ETA，国外仍使用 TomTom；Egern 保持固定值 | 改为 `Apple` 后中国大陆导航可能不可用；Egern 为保持已验证效果不提供修改入口 |
...
| `Hybrid.ServiceMode` | `CN_POI` | 国内地点、POI 和反向地理编码使用 CN 服务，导航遵循 `Directions`；国际 Munin/四处看看入口固定保留 | `APPLE` 更偏国际服务、国内数据可能减少；`CN_FULL` 会进一步使用 CN 交通服务 |
...
| 客户端 | 参数样式 | 说明 |
| --- | --- | --- |
| Egern | `compat_arguments` + `compat_arguments_desc` | 保持此前验证通过的模块与固定响应脚本；导航固定为 AutoNavi |
| Surge | `#!arguments` + `#!arguments-desc` | 全部可配置参数均显示在参数面板并附说明 |
| Loon | `[Argument]` | 每个可配置项均带 `tag`、`desc` 和默认值 |
| Shadowrocket | 模块注释 + Script `argument` | 格式无统一交互参数面板；头部逐项列出全部默认值、可选方向和修改影响，可手动修改 argument |
| Stash | YAML 注释 + Script `argument` | 头部逐项列出全部默认值、作用和修改影响，可直接修改各脚本 argument |
| Quantumult X | snippet 注释 + bundle 内置参数 | 无交互参数面板；头部逐项列出全部内置默认值、作用和修改影响 |
...
3. 确认只启用一份 Maps 混合模块。
4. Egern 用户确认响应脚本使用模块中固定的提交地址；其他客户端脚本缓存参数应为 `v=6.4.0`。
...
- `CN_POI`：默认值。中国地点、POI、搜索和反向地理编码使用 CN 服务；导航由 `Directions` 决定，所有客户端默认 AutoNavi，国外仍使用 TomTom。
...
| Mainland 3D Route v6 / module 6.4.0 | **当前稳定版** | 除 Egern 外统一恢复国内高德导航并保留国际四处看看；国内外 2D 卫星、国外 3D、国内 POI 与跨地区直接切换保持不变 |
...
- 日本、韩国等地区的 Look Around（四处看看）暂未实现，正在排查中；其他已测试的国际城市（如洛杉矶、旧金山、伦敦、巴黎、柏林、纽约）四处看看正常。
- 中国大陆卫星地图影像与标准地图图层之间仍存在坐标漂移，尚未找到可在设备上验证有效的修复方案，正在持续排查中。
